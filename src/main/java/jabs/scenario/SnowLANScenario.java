...
public class SnowLANScenario extends AbstractScenario {
    ...
    protected void insertInitialEvents() {
        ...
        SnowBlock snowBlock = BlockFactory.sampleSnowBlock(simulator, network.getRandom(), (SnowNode) startNode, SNOW_GENESIS_BLOCK);
        snowNode.currentBlock = snowBlock;
        for (Node destination : sampledNeighbors) {
            startNode.query(new QueryMessage(new SnowPrepareQuery<>(startNode, snowBlock)), destination);
        }
        //-----------------------------------
        for (int n = 1; n < nodes.size(); n++) {
            ...
            for (Node destination : samples) {
                nodes.get(n).query(new QueryMessage(new SnowPrepareQuery<>(nodes.get(n), SNOW_GENESIS_BLOCK)), destination);
            }
        }
    }
